			...
			function price_calculator(ship_fee){
				...
				$('.acessories_price_item').each(function () {
					...
					if( jQuery(this).val() ) 
					{
						...
						var acessories_option_selected = acessories_price_item_element.find('.acessories_dropdown_list option:selected').val();
						
						var acessories_item_cost_price = round_2_digits(  Number( jQuery(this).val() ) );
						
						var acessories_item_markup_value = Number( acessories_price_item_element.find('.acessories_markup_select').val() );
						...
						var acessory_cost_price = acessories_option_selected ? round_2_digits( Number( acessories_option_selected ) * qty ) : 0;
						var acessory_list_price = acessories_option_selected ? round_2_digits( Number( acessories_price_item_element.find(' acessories_dropdown_list option:selected').attr('data-list-price') ) * qty ) : 0;
						var acessory_retail_percent = acessories_option_selected ? round_2_digits( Number( acessories_price_item_element.find('acessories_dropdown_list option:selected').attr('data-retail_percent') ) ) : 0;
						
						var acessory_retail_price = acessories_option_selected ? round_2_digits( acessory_list_price * (1 - (acessory_retail_percent/100)) ) : 0;
						...
						listprice = round_2_digits( listprice + acessories_item_markup_price   );
						totalCost += round_2_digits( acessories_item_cost_price  );
						clientPrice += round_2_digits( acessories_item_retail_price );

						print_cost_table_sub +=  '<tr>' + 
													'<td> Acessories </td>' +
													'<td> $' + acessories_item_retail_price + ' </td>' +
